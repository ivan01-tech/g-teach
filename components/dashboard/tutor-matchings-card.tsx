...
import {
  setAllMatchings,
  setLoading,
  acceptMatchingAction,
  refuseMatchingAction,
...
export function TutorMatchingsCard() {
  ...
  const { allMatchings: matchings, loading } = useAppSelector(
  ...
  const activeMatchings = matchings.filter(
  ...
  const handleRefuse = async (id: string) => {
    await dispatch(refuseMatchingAction({ matchingId: id, reason: "" }));
  };
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "confirmed":
        return (
          <Badge className="bg-green-500/10 text-green-600">Confirmé</Badge>
        );
      default:
        ...
        <div className="space-y-4">
                  ...
                  <p className="text-xs text-muted-foreground flex items-center gap-1">
                    ...
                    {matching.contactDate
                      ? Math.floor(
                          (Date.now() -
                            (
                              matching.contactDate?.toDate?.() ||
                              new Date(matching.contactDate)
                            ).getTime()) /
                            (1000 * 60 * 60 * 24),
                        )
                      : 0}{" "}
              ...
              <div className="flex items-center gap-2">
                ...
                {matching.status === "requested" ? (
                  ...
                  <Button size="sm" variant="ghost" asChild>
                    <Link
                      href={`/betreuer/messages?student=${matching.learnerId}`}
                    >
          ...
          {activeMatchings.length > 5 && (
            <Button variant="link" className="w-full" asChild>
              <Link href="/betreuer/messages">Voir tous les messages</Link>
            </Button>
          )}
        </div>
      </CardContent>
